...
`;
    ...
    setTimeout(() => { // se repite  1 vez
      ...
      if (addButton != undefined) {
        ...
        const buttonsVideo = document.querySelector('#top-row.ytd-watch-metadata'); // Botones para video
    ...
    async function cargarDislikes() {
      ...
      if (validoUrl.split('/')[3] !== 'shorts' && validoVentana != undefined && validoUrl != 'https://www.youtube.com/') {
        ...
        try {
          ...
          while (startRating.firstChild) { // limpiar HTML previed - previo
            startRating.removeChild(startRating.firstChild);
          } // Rating califiacion del video
          for (let i = 1; i <= Math.round(start); i++) { // Add stars to video
    ...
    setInterval(() => {
      ...
      if (currUrl != prevUrl) {
        ...
        const formulariodescarga = document.querySelector('.formulariodescarga');
        setTimeout(() => {
          if (formulariodescarga != undefined) { formulariodescarga.classList.add('ocultarframe'); }}, 500)
          prevUrl = currUrl;
          setTimeout(() => {
            cargarDislikes();
            shortDislike();
